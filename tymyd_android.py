...
def doLookMian(friendUserId):
	...
	url = "https://apps.game.qq.com/wuxia_cgi/ZoneTranspondCgi/index.php?act=requestTranspond&sMethodPath=/api/user/getuserinfo&callback=jQuery17100012562480264131093_{}&uin=1192769569&userId=7904671&friendUserId={}&_={}"
	headers = {
	    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.149 Safari/537.36',
	    'Cookie':COOKIE
	}
	base_url = url.format(nowtime,friendUserId,nowtime)
...
def doPinglun(momentId):
	...
	url = "https://apps.game.qq.com/wuxia_cgi/ZoneTranspondCgi/index.php?act=requestTranspond&sMethodPath=/api/moment/addcomment&callback=jQuery17108402774150402388_1590{}&uin=1192769569&userId=7904671&roleId=1318055637&text=1&momentId={}&replyCommentId=0"
	headers = {
...
def getBefriends():
	nowtime = int(time.time() * 1000)
	url = "https://apps.game.qq.com/wuxia_cgi/ZoneTranspondCgi/index.php?act=requestTranspond&sMethodPath=/api/user/getbefriends&callback=jQuery17106452051136608634_{}&uin=1192769569&friendUserId=7904671&lastIndex=null&_={}"
	headers = {
	...
	base_url = url.format(nowtime,nowtime)
	req = request.Request(base_url,headers=headers)
	try:
		response = request.urlopen(req)
		responseText = response.read().decode("utf-8")[41:-2]
		...
		#获取最新10个
		for i in range(10):
			if (str(data_list[i]['userId'])+"\n") not in getTxtid():
				inputTxtid(data_list[i]['userId'])
...
#关注好友
def doAddfriend(userId):
	time.sleep(1)
	nowtime = int(time.time() * 1000)
	url = "https://apps.game.qq.com/wuxia_cgi/ZoneTranspondCgi/index.php?act=requestTranspond&sMethodPath=/api/user/addfriend&callback=jQuery17108775488098925086_{}&uin=1192769569&userId=7904671&friendUserId={}&_={}"
	headers = {
	    'User-Agent': 'com.tencent.gamehelper.wuxia/2103030265 (Linux; U; Android 6.0.1; zh_CN; MI 4LTE; Build/MMB29M; Cronet/72.0.3626.96)',
	    'Accept-Encoding':'gzip, deflate, br',
	    'Cookie':COOKIE
	...
	try:
		...
		returnMsg = json.loads(json.loads(responseText)['jData']['jData'])['returnMsg']
		print(returnMsg)
		print("--------关注[{}]--------异常:{}".format(userId,Fore.RED+returnMsg))
	except Exception as r:
		print('未知错误 %s' %(Fore.RED+str(r)))
